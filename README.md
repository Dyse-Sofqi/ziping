...
- 点击面板/侧边栏/代码块中的流年 → 文档光标自动跳转到对应的 `\t- {year}年` 行
...
| 版本 | 说明 |
|------|------|
| 1.4.3 | 修复：自动激活视图时由标签页栏改为右侧边栏打开（`getLeaf` → `getRightLeaf`），对齐 manifest views 声明 |
| 1.4.2 | 修复：新标签页打开时左侧面板闪现偏移（面板初始化时先显示空面板再隐藏，导致正文右移闪烁） |
| 1.4.1 | 修复：补齐 selectFolder() 后缺失的 eslint-enable 指令 |
| 1.4.0 | 设置：案例保存路径横向布局（文本框+清空+浏览按钮）、原生文件夹对话框、仓库名/相对路径显示、文件夹重命名自动同步路径 |
| 1.3.2 | 修复：子节点点击时逆向搜索流年行 |
